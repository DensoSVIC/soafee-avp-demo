  ...
  tf_listener(tf_buffer, std::shared_ptr<rclcpp::Node>(this, [](auto) {}), false)
{
  ...
  // FIXMESB: gets the goal pose from rviz2...
  goal_pose_sub_ptr =
  ...
  // FIXMESB: Don't request the binary map over ROS. We take care of it in LF.
  // this->request_osm_binary_map();
}

Lanelet2GlobalPlannerNode::Lanelet2GlobalPlannerNode(
  const rclcpp::NodeOptions & node_options,
  void* set_global_route,
  int (*schedule_value)(void*, int64_t, void*, int))
: Lanelet2GlobalPlannerNode(node_options)
{
  this->set_global_route = set_global_route;
  ...
  const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  ...
  if (lanelet2_global_planner->plan_route(start, end, route)) {
    ...
    // FIXMESB: stamping the message with ROS clock, which can differ from LGSVL timestamp
    // if its behind. Can't change the goal once a goal is chosen until the destination is reached.
  ...
  const autoware_auto_msgs::msg::TrajectoryPoint & end_point, const std_msgs::msg::Header & header)
{
  ...
  m_global_route = global_route;
  auto msg_copy = new autoware_auto_msgs::msg::Route(m_global_route);
  this->schedule_value(this->set_global_route, 0, static_cast<void*>(msg_copy), 1);
}
