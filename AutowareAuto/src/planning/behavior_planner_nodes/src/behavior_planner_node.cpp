...
void BehaviorPlannerNode::init()
{
  ...
  // Don't create clients for the sub-planner services. We will use Xronos to
  // communicate with these planners.
  ...
  // (Don't) wait until action clients are ready
  ...
  // Don't create a client for the HAD map service. We will deliver this map
  // on the Xronos side.
  ...
  // Don't create a client for the object collision estimator. This is handled
  // on the Xronos side.
  ...
  // Setup subscribers
  // FIXMESB: with the exception of async action calls, store on route and
  // vehicle_state_report and publish trajectory on vehicle state (kinematic state)

  // Vehicle Kinematic State coming from the vehicle interface
  m_ego_state_sub = this->create_subscription<State>(
    "vehicle_state", QoS{10},
    [this](const State::SharedPtr msg) {on_ego_state(msg);});
  // Planning global path coming from lanelet2_global_planner_node in AVP demo
  m_route_sub = this->create_subscription<Route>(
    "route", QoS{10},
    [this](const Route::SharedPtr msg) {on_route(msg);});
  // Vehicle State Report coming from the vehicle interface
  ...
  // FIXMESB: vehicle state command is published directly by the behavior planner
  // only used when changing the gear
...
void BehaviorPlannerNode::on_ego_state(const State::SharedPtr & msg)
{
  ...
  // FIXMESB: discards vehicle state if localization data is not available
  ...
  // If we have arrived at the goal. Put car in park and set the current goal
  // to the current position.
  if (m_planner->has_arrived_goal(m_ego_state)) {
    m_state_command = std::make_shared<VehicleStateCommand>();
    m_state_command->gear = VehicleStateCommand::GEAR_PARK;
    m_state_command->blinker = VehicleStateCommand::BLINKER_OFF;
    m_state_command->mode = VehicleStateCommand::MODE_AUTONOMOUS;
    m_state_command->stamp = msg->header.stamp;

    m_trajectory = std::make_shared<Trajectory>();
    m_trajectory->header.frame_id = "map";
    m_trajectory->header.stamp = msg->header.stamp;
    m_trajectory->points.push_back(msg->state);

  } else if (desired_gear != m_current_gear) {
    const auto throttle_time = std::chrono::duration<float64_t>(3);
    ...
    m_state_command = std::make_shared<VehicleStateCommand>(gear_command);
    ...
    m_trajectory = std::make_shared<Trajectory>(trajectory);
    ...
    //       this, std::placeholders::_1));
      m_trajectory_to_modify = std::make_shared<Trajectory>(trajectory);
    // } else {
    //   // m_trajectory_pub->publish(trajectory);
    //   m_trajectory = std::make_shared<Trajectory>(trajectory);
    // }
...
{
  // FIXMESB: store on this function
  // FIXMESB: only the gear is used in the vehicle state report
  m_current_gear = msg->gear;
}

void BehaviorPlannerNode::on_route(const Route::SharedPtr & msg)
{
  // Currently, route is published once per a goal designated in rviz2. This goal/route appears
  // to not change until the previous goal is reached and a new goal is chosen.
  if (m_requesting_trajectory) {
    RCLCPP_ERROR(
      get_logger(),
  ...
  // FIXMESB: no synchronized service request
  // auto result =
...
{
  // FIXMESB: once trajectory is modified through the object collision estimator
...
void BehaviorPlannerNode::map_response(rclcpp::Client<HADMapService>::SharedFuture future)
{
  ...
  // FIXME: storing the route and not acting on it
