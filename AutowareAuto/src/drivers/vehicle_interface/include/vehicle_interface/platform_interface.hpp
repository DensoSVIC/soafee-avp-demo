...
public:
  ...
  // Stores the vehicle kinematic state output of the vehicle interface, so that
  // it can be read in Xronos and sent over the Xronos interface.
  std::shared_ptr<autoware_auto_msgs::msg::VehicleKinematicState> m_vse_t;

  // LF stuff
  void* m_kinematic_state_action; // Physical action
  void* m_state_report_action;    // Physical action
