  ...
  const std::chrono::nanoseconds & cycle_time)
{
  ...
  // Timer
  // Don't need this timer since its only job is to publish vehicle state and
  // odometry. We will do the publishing in Xronos.
  ...
  //   });
  // Make publishers
  // FIXMESB: Tolerate dangling publishers for now to minimize the changes to
  // Autoware nodes.
  ...
  m_state_sub = create_subscription<VSC>(
    ...
    [this](VSC::SharedPtr msg) {
      // FIXMESB: store vehicle state command coming from the behavior planner (concerning gear change),
      // act on vehicle command coming from the mpc controller
      m_last_state_command = *msg;});
...
////////////////////////////////////////////////////////////////////////////////
// Periodically read the vehicle state and odometry and publish it
void VehicleInterfaceNode::read_and_publish()
{
  ...
  // Update the state machine (FIXMESB: might not be needed in Xronos)
  if (m_state_machine) {
