...
{
////////////////////////////////////////////////////////////////////////////////
ControllerBaseNode::ControllerBaseNode(const std::string & name, const std::string & ns, rclcpp::NodeOptions nodeOptions)
: Node{name, ns, nodeOptions}
{
  ...
  const std::string & diagnostic_topic)
{
  ...
  // FIXMESB: compute on state, trajectory, and tf
...
void ControllerBaseNode::retry_compute()
{
  ...
  // throughput
  // FIXMESB: In Xronos, trajectory is always aligned with the kinematic state,
  // so the size of this buffer will always be 1.
...
bool ControllerBaseNode::try_compute(const State & state)
{
  ...
  // FIXMESB: Possible expensive transform because kinematic state is not
  ...
  if (m_tf_buffer.canTransform(traj_frame, state_frame, stamp)) {
    ...
    // RCLCPP_WARN(
    //   get_logger(),
    //   "try_compute: Cannot transform from the trajectory frame to the vehicle state frame.");
    return false;
  }
  ...
  // Compute result
  try {
    auto cmd{m_controller->compute_command(state_tf)};
    publish(cmd);
    diagnostic_fn();
  } catch (...) {
    diagnostic_fn();
...
////////////////////////////////////////////////////////////////////////////////
void ControllerBaseNode::publish(Command & msg)
{
  // m_command_pub->publish(msg);
  this->cmd = msg;
  this->cmd_is_set = true;
}
